#!/usr/bin/env python3
"""
Generate videos from Phase 2 trained model.

This script:
1. Loads a Phase 2 checkpoint (tokenizer, dynamics, heads)
2. Takes input frames from the dataset
3. Uses the dynamics model to predict future frames
4. Decodes latents back to video frames
5. Saves as video file
"""
...
from eval_phase2 import load_config, create_tokenizer, create_dynamics_model, create_heads, load_phase2_checkpoint
...
def decode_latents_to_frames(tokenizer, latents):
    ...
    for t in range(T):
        ...
        # The tokenizer decode method expects (B, num_latent, latent_dim)
        try:
    ...
    with torch.inference_mode():
        ...
        for step in range(rollout_steps):
            ...
            if step < actions.shape[1]:
                ...
                # Get last timestep's latent
                last_latent = latents_flat[:, -1:]  # (B, 1, input_dim)
            ...
            # We need to provide latents and actions
            # For simplicity, use the last latent and predict next
            ...
            # Dynamics expects latents (B, T, num_latent, latent_dim) and actions (B, T)
...
def save_video(frames: torch.Tensor, output_path: str, fps: int = 10, codec: str = 'libx264'):
    ...
    if HAS_IMAGEIO:
        try:
            # imageio uses ffmpeg which has better codec support
            # Default to libx264 (H.264) which is most compatible
...
def main():
    parser = argparse.ArgumentParser(description="Generate videos from Phase 2 model")
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
    ...
    print_flush(f"Checkpoint: {args.checkpoint}")
